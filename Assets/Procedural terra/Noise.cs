...
public static class Noise
{
    ...
    float persistance, float lacunarity, Vector2 offset,NormalizeMode normalizeMode)
    {
        ...
        Vector2[] octavOffsets = new Vector2[octaves];
        ...
        for(int i = 0; i < octaves; i++)
        {
            ...
            octavOffsets[i] = new Vector2(offsetX,offsety);
        ...
        for(int y = 0; y < mapHeight;y++)
        {
            for(int x = 0; x < mapWidth;x++)
            {
                ...
                for(int i = 0; i < octaves;i++)
                {
                    float sampleX = (x-halfWidth + octavOffsets[i].x) / scale * frequency;
                    float sampleY = (y-halfHeight + octavOffsets[i].y) / scale * frequency;
